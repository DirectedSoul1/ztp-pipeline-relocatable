...
function extract_vars() {
    ...
    for node in $(oc --kubeconfig=${SPOKE_KUBECONFIG} get nodes -o name | cut -f2 -d/); do
        nodes+="${node},"
    done
...
if ! ./verify.sh; then
    ...
    for spoke in ${ALLSPOKES}; do
        ...
        for crd in ${LSOCRDS[@]}
        do
            check_resource "crd" "${crd}" "Established" "openshift-local-storage" "${SPOKE_KUBECONFIG}"
        done
        ...
        echo ">>>> Waiting for: LSO LocalVolume on ${spoke}"
        echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
        check_resource "localvolume" "localstorage-disks-block" "Available" "openshift-local-storage" "${SPOKE_KUBECONFIG}"
